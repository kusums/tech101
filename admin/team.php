...
if (isset($_POST['submit'])) {
    ...
    if (!empty($name) && !empty($desc)  && !empty($img)) {
        ...
        $str = "SELECT * FROM `tbl_team` where `name`='" . $name . "'";
        $result = mysqli_query($con, $str);
        $number = mysqli_num_rows($result);
        if ($number > 0) {
            $_SESSION['failure']['message'] = "Team already exists.";
        } else {
            ...
            if ($uploadOk == 0) {
                ...
                if ($uploadimg) {
                    ...
                    $query = "INSERT INTO `tbl_team` SET
                                    name = '" . $name . "',
                                    description = '" . $desc . "',
                                    image = '" . $newfilename . "'
    ...
    <div class="col-md-12">
        <div class="password-panel">
            <div class="login-head">
                <h1 class="text-center">Add Team Members</h1>
            </div>
            <div class="login-body">
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <button class="btn btn-primary" type="submit" name="submit">Add Members</button>
                </form>
            </div>
        </div>
    </div>
